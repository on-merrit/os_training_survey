...
```{r, e1, fig.width=20, fig.height=0.005}

e1_rec_df <- df %>%
  select("E1_rec")

e1_rec_df %>%
plot_bar(E1_rec, title = "Countries", reorder = TRUE, nudge_y = .05)
...
e2_df_rec %>% 
  plot_bar(E2, title = "Gender", reorder = FALSE, nudge_y = .028)
```
...
e3b_df %>% 
  plot_bar(E3b, title = "Publish first academic publication", reorder = FALSE, nudge_y = .028)
...
e4_df_rec %>%
  plot_bar(E4, title = "Highest education", reorder = FALSE, nudge_y = .028)
```
...
e5_df_rec %>% 
  plot_bar(E5, title = "Types of institution", reorder = TRUE, nudge_y = .028)
...
e6_df_rec %>% 
  plot_bar(E6, title = "Position", reorder = TRUE, nudge_y = .028)
```
...
e7_df_rec %>% 
  plot_bar(E7, title = "Respondents by disciplines", reorder = TRUE, nudge_y = .028)
