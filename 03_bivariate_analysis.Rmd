...
country_b10_df <- df %>% 
  select(starts_with("B10["), E1_rec) %>% 
  mutate(across(starts_with("B10["), .fns = factor, levels = answer_levels_3)) %>% 
  mutate(E1_rec = forcats::fct_lump_min(E1_rec, min = 4, other_level = "Other")) %>%
  filter(E1_rec != "NA")
...
country_c8_df <- df %>% 
  select(starts_with("C8["), E1_rec) %>% 
  mutate(across(starts_with("C8["), .fns = factor, levels = answer_levels_7)) %>% 
  mutate(E1_rec = forcats::fct_lump_min(E1_rec, min = 4, other_level = "Other")) %>%
  filter(E1_rec != "NA")
